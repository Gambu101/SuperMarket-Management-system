...
app.post("/api/signup", async (req, res) => {
  ...
  try {
    // Frontend supports: admin | staff
    // Backend DB supports: user | manager | admin
    let dbRole = String(rawRole).toLowerCase();

    if (dbRole === "staff") dbRole = "manager";

    // Safety: allow only known DB roles
    if (!["user", "manager", "admin"].includes(dbRole)) {
      dbRole = "user";
    }
    ...
    await pool.query(
      ...
      [username, firstname, lastname, email, hashedPassword, dbRole],
    );
...
// Only managers/admin can add/restock inventory.
app.post(
  "/api/inventory",
  authenticateToken,
  requireManager,
...
// GET /api/stock-activities - user actions (inventory changes)
// - user: only their own actions
// - manager/admin: all actions
app.get("/api/stock-activities", authenticateToken, async (req, res) => {
  try {
    ...
    if (role === "admin" || role === "manager") {
      ...
      res.json(
        rows.map((r) => ({
          ...r,
          details:
            typeof r.details === "string" ? tryParseJSON(r.details) : r.details,
        })),
      );
      return;
    }
